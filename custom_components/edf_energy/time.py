...
from .intelligent.target_time import EDFEnergyIntelligentTargetTime
...
async def async_setup_intelligent_sensors(hass, config, async_add_entities):
  ...
  for intelligent_device in intelligent_devices:
    ...
    if intelligent_features.ready_time_supported:
      entities.append(EDFEnergyIntelligentTargetTime(hass, settings_coordinator, client, intelligent_device, account_id))
