...
The integration polls and retrieves data at different intervals depending on the target data. Below is a rough table describing how often the integration targets refreshing various bits of data. This has been done to try and not overload the API while also providing useful data in a timely fashion.
...
| Smart Charging tariff based sensors | 3 | Trying to balance refreshing settings and new dispatch information without overloading the API |
| Smart Charging device information | 360 (6 hours) | Shouldn't change often, but want to be alerted when it does as sensors will stop working |
...
EDF Energy provide rate information for Smart Charging tariffs. Unfortunately, this rate information is only the standard rate information (e.g. standard peak and off peak rates). This means that periods that are charged at off peak rates outside of the normal off peak period has to be adjusted to the off peak rate manually. Luckily, EDF Energy provides information on planned and completed dispatches. However, there are a few issues with this data
...
Therefore, there is the concept of started dispatches. Once data is refreshed, any planned dispatches that are active become a started dispatch, thirty minute increments at a time. This is because the whole thirty minute period for a smart charge dispatch is charged at off peak, even if only a minute charge has occurred. It is done thirty minute increments at a time because EDF Energy might stop a dispatch early. This will only occur if the [Smart Charging state](./entities/intelligent.md#current-state) is in `SMART_CONTROL_IN_PROGRESS`. For example if you have a planned dispatch of `2025-04-01T10:00:00`-`2025-04-01T11:00:00`, at `2025-04-01T10:01:00` if the planned dispatch is still available the period of `2025-04-01T10:00:00`-`2025-04-01T10:30:00` will be added.
...
## I'm on a Smart Charging tariff but the off peak sensor is only coming on during standard off peak times. Is there something wrong?

For Smart Charging tariffs, this sensor will turn on when you're within the standard off peak rates or within an active ad hoc dispatching period.
...
This above does not apply and will not come on outside of the standard off peak hours for the following Smart Charging providers

* OHME

Assuming you're with a valid Smart Charging provider, another reason might be you've turned on [manual dispatch refetches](./setup/account.md#manually-refresh-intelligent-dispatches), which turns off automatic refreshing. If you're not calling the service (either manually or via the provided [blueprint](./services.md#edf_energyrefresh_intelligent_dispatches)), then the dispatches won't be available to adjust the off peak sensor.

If you are with an accepted provider and are getting dispatch information, then you can use this [available service](./services.md#edf_energyget_point_in_time_intelligent_dispatch_history) to see what state the dispatches are at a given point in time when you believe the off peak sensor should be on. If you are seeing dispatches available based on your settings and still having issues, please raise an issue.
...
1. Go to the [EDF Energy](https://my.home-assistant.io/redirect/integration/?domain=edf_energy) integration.
2. Click on `entities`
...
* The serial number and mpan/mprn is present because the data that comes back from EDF Energy is in an array of arrays. I am only on an import tariff, so couldn't guarantee that any of these were unique and didn't want to assume anything. Turns out one of these is duplicated between import/export meters so if I had picked one of these I would have been wrong and had a clash.
...
> Update of sensor.edf_energy_xxx is taking over x seconds

> Updating edf_energy sensor took longer than the scheduled update interval
...
2. EDF Energy APIs are slow to respond, or having issues.

If you wish to suppress this warning, you can follow [this advice](https://github.com/stevekirtley/HomeAssistant-EDFEnergy/issues/429#issuecomment-1783739547).
...
The identifiers of the entities should then be checked against your EDF Energy dashboard to verify the correct entities are being picked up. If this is producing unexpected results, then you should raise an issue.
...
* The current consumption (m3) is always calculated, as the API provides this data in kWh.
...
## My tariff has vanished from the API and my rates show as unknown. What's going on?
...
where states_meta.entity_id = 'binary_sensor.edf_energy_xxx_intelligent_dispatching'
...
If you've installed the integration manually, then you should keep an eye on the [GitHub releases](https://github.com/stevekirtley/HomeAssistant-EDFEnergy/releases). You could even subscribe to the [RSS feed](https://github.com/stevekirtley/HomeAssistant-EDFEnergy/releases.atom).

## There's a beta release of the integration that I would like to take part in, how do I do this?

If you install the integration manually, it's just a case of getting the source of the [beta release](https://github.com/stevekirtley/HomeAssistant-EDFEnergy/releases), replacing the old installation with the new one and restarting Home Assistant.

If you install the integration via HACS, then you will need to

* Go to [HACS](https://my.home-assistant.io/redirect/hacs_repository/?owner=stevekirtley&repository=HomeAssistant-EDFEnergy&category=integration), click on the three dots and then click redownload
...
1. Navigate to the [EDF Energy integration](https://my.home-assistant.io/redirect/integration/?domain=edf_energy)
