...
!!! warning "Not available with EDF Energy"
    ...
    EDF Energy's API does **not** provide a live/current-day consumption feed (unlike Octopus Energy's Home Mini), so this integration does not create any live consumption entities — `current_consumption`, `current_demand`, `current_accumulative_consumption`, `current_total_consumption`, `current_total_export`, `current_accumulative_cost` or `current_interval_accumulative_consumption`. Older versions of these docs listed them; they were inherited from upstream and never populated on EDF.

    To feed today's usage into the Energy dashboard, use an independent live source such as Hildebrand Bright / Glow or a CT clamp. See [Energy Dashboard → Current Consumption](../setup/energy_dashboard.md#current-consumption) for the full setup.
